...
`requirements.txt` pins the versions it ran under, on Python 3.13; the map export
(`scripts/export_site.py`) also needs Node 20.11 or later and mapshaper 0.7.61.
[METHODS.md](METHODS.md) describes each stage, the matching rules in order and what the tests
check. Run the tests with `python -m pytest tests`: they check the published files against the
rules, and skip the ones that need a raw snapshot, which is not published.
...
- **The license files now hold the standard texts.** `LICENSE` holds the SPDX text of CC BY 4.0 and
  `LICENSE-CODE` the MIT license, each on its own, so a tool can identify them; the old single file
  was a composite that none could.
